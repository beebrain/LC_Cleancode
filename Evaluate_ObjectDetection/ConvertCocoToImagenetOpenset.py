...
###### list os to file
data_LC_closeset = pd.read_csv("./ResultObjectMap/Lecog_ResultOpenset_fill20Class.txt")

OBject_result = pd.DataFrame()
for _,row_LC in data_LC_closeset.iterrows():
    fileName = row_LC["lowLevelName"]
    raw_data = pd.read_csv("./ResultObjectMap/Openset/{}.csv".format(fileName))
    print(fileName)
    selectedclass = False
    dataPredict = {}
    ### They can't detect any object in image. That mean they predict unseen class
    # print(len(raw_data))
    if len(raw_data) == 0:
...
OBject_result.to_csv("./ResultObjectMap/Obj_resultOpenset_fill20Class.txt")